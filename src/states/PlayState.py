...
import settings
import src.powerups
import src.powerups as PowerUp_Rain
...
class PlayState(BaseState):
    def enter(self, **params: dict):
        ...
        #self.PowerUp_Rain = params.get("PowerUp_Rain", [-1])
        ...
        for ball in self.balls:
            ...
            if random.random() < 0.333:
                ...
                self.powerups.append(
                  ...
                  # Chance to balls Rain        
            if random.random() < 0.333:
                r1 = brick.get_collision_rect()
                self.powerups.append(
                    self.powerups_abstract_factory.get_factory("BallRain").create(
                        r1.centerx - 8, r1.centery - 8
                    )
                )

                # Chance to add a   troll mode  deactivate    
            if random.random() < 0.00:
                r2 = brick.get_collision_rect()
                self.powerups.append(
                    self.powerups_abstract_factory.get_factory("Stickii").create(
                        r2.centerx - 8, r2.centery - 8
                    )
                )
            ...
            if random.random() < 0.333:
                r2 = brick.get_collision_rect()
                self.powerups.append(
                    self.powerups_abstract_factory.get_factory("Missiles").create(
                        r2.centerx - 8, r2.centery - 8
                    )
                )    
